  ...
  return (
    <main className="flex-1 bg-background p-5 text-foreground md:p-8">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <Card><CardHeader><CardTitle>Administration areas</CardTitle><CardDescription>These settings apply across every workspace unless an allowed lower-level override exists.</CardDescription></CardHeader><CardContent className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{globalAdministrationSections.map((section: string) => <div key={section} className="rounded-md border border-border bg-card p-3 text-sm text-card-foreground">{section}</div>)}<div className="rounded-md border border-border bg-card p-3 text-sm text-card-foreground"><a href="/admin/audit-log" className="flex items-center gap-2 hover:underline"><ShieldCheck className="size-4" />Audit Log</a></div></CardContent></Card>
